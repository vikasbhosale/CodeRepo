...
//            pstmt_field=getConn().prepareStatement("Select * from "+schema_name+"."+table_name);
    ...
    private void sub_DelExeActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_sub_DelExeActionPerformed
        try {
            ...
            DefaultTableModel tbmod2=(DefaultTableModel)fieldtable.getModel();            
            String setvalue="";
            for(int i=0;i<fieldtable.getRowCount();i++)
            {
                if(!tbmod2.getValueAt(i, 1).equals(""))
                {
                    setvalue=setvalue+tbmod2.getValueAt(i, 0).toString()+" = "+tbmod2.getValueAt(i, 1).toString()+",";
                }
            }
